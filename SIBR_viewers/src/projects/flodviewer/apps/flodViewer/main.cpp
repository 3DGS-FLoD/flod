...
std::string findLargestPlyFile(const std::string& directoryPath, size_t maxSizeInMB) {
    ...
    size_t maxSizeInBytes = maxSizeInMB * 1024 * 1024;
    size_t largestFileSize = 0;
    std::string largestPlyFile;
    ...
    for (auto& entry : fs::recursive_directory_iterator(dirPath)) {
        if (fs::is_regular_file(entry) && entry.path().extension() == ".ply") {
            ...
            if (fileSize <= maxSizeInBytes && fileSize > largestFileSize) {
                largestFileSize = fileSize;
                largestPlyFile = entry.path().string();
            }
        }
    }

    return largestPlyFile;
}
...
int main(int ac, char** av) 
{
	...
	std::string plyfile = findLargestPlyFile(myArgs.modelPath.get() + "/point_cloud_viewer/", myArgs.maxSize.get());
