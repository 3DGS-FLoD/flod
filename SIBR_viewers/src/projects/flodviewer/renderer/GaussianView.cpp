...
#include <projects/flodviewer/renderer/GaussianView.hpp>
...
std::string getBaseDir(const std::string& filename) {
    size_t pos = filename.find("/level/level_");
    if (pos == std::string::npos) {
        pos = filename.find("/selective/levels_");
    }

    if (pos != std::string::npos) {
		std::string baseDir = filename.substr(0, pos);
        std::cout << "Base Directory: " << baseDir << std::endl; // Print the base directory
        return filename.substr(0, pos);
    }

    // If not found, return an empty string or handle the error as needed
	std::cout << "Base Directory not found" << std::endl; // Print error message
    return "";
}
	...
	sibr::ViewBase(render_w, render_h)
{
	...
	// Extract the base directory from the provided filename
    ...
    if (lastSlashPos != std::string::npos) {
        parentDirectory = filePath.substr(0, lastSlashPos);
        fileName = filePath.substr(lastSlashPos + 1);
    } else {
        parentDirectory = ""; // Handle the case where there's no directory in the path
        fileName = filePath; // The file path itself is the file name
    }

    // Further extract the immediate parent directory name (level or selective)
    size_t parentDirSlashPos = parentDirectory.find_last_of("/\\");
    if (parentDirSlashPos != std::string::npos) {
        parentDirectory = parentDirectory.substr(parentDirSlashPos + 1);
    }

    // Determine rendering mode based on parent directory
    if (parentDirectory == "selective") {
        _selectiveRendering = true;
        _levelRendering = false;

        // Extract the render level subset from the file name
        if (fileName.find("levels_5_4_3") != std::string::npos) {
            renderLevel = "{5,4,3}";
        } else if (fileName.find("levels_4_3_2") != std::string::npos) {
            renderLevel = "{4,3,2}";
        } else if (fileName.find("levels_3_2_1") != std::string::npos) {
            renderLevel = "{3,2,1}";
        }
    }
    else if (parentDirectory == "level") {
        _selectiveRendering = false;
        _levelRendering = true;

        // Extract the render level from the file name
        if (fileName.find("level_5") != std::string::npos) {
            renderLevel = "5";
        } else if (fileName.find("level_4") != std::string::npos) {
            renderLevel = "4";
        } else if (fileName.find("level_3") != std::string::npos) {
            renderLevel = "3";
        } else if (fileName.find("level_2") != std::string::npos) {
            renderLevel = "2";
        } else if (fileName.find("level_1") != std::string::npos) {
            renderLevel = "1";
        }
    }
	// Now that renderLevel is set, initialize previousRenderLevel with its value
...
void sibr::GaussianView::onGUI()
{
	...
	const std::string guiName = "3D Gaussians - FloD";
	...
	if (currMode == "Splats")
	{
		// Add separted section title "FloD options"
		ImGui::Dummy(ImVec2(0.0f, 10.0f));
		ImGui::Separator();
        ImGui::Text("FloD options");

        // Add Level rendering and Selective rendering checkboxes side by side with radio button-like behavior
        bool levelRendering = _levelRendering;
        bool selectiveRendering = _selectiveRendering;

        if (ImGui::Checkbox("Level rendering", &levelRendering))
        {
            if (levelRendering)
            {
                _levelRendering = true;
        ...
        ImGui::SameLine();
        if (ImGui::Checkbox("Selective rendering", &selectiveRendering))
        {
		...
		if (_levelRendering)
		{
			if (ImGui::BeginCombo("Render Level", renderLevel.c_str()))
			{
				if (ImGui::Selectable("5"))
					renderLevel = "5";
				...
				if (ImGui::Selectable("1"))
					renderLevel = "1";
				ImGui::EndCombo();
				onRenderLevelChanged(renderLevel);
			}
		} 
		if (_selectiveRendering) 
		{
			if (ImGui::BeginCombo("Render Level Subset", renderLevel.c_str()))
			{
				if (ImGui::Selectable("{5,4,3}"))
					renderLevel = "{5,4,3}";
				if (ImGui::Selectable("{4,3,2}"))
					renderLevel = "{4,3,2}";
				if (ImGui::Selectable("{3,2,1}"))
					renderLevel = "{3,2,1}";
				ImGui::EndCombo();
				onRenderLevelChanged(renderLevel);
			}
...
void sibr::GaussianView::onRenderLevelChanged(const std::string& renderLevel)
{

	// Check if the render level has changed
    if (renderLevel == previousRenderLevel) {
        return; // Do nothing if the render level hasn't changed
    }

    // Update the previous render level to the current one
    previousRenderLevel = renderLevel;
	...
	std::string filename;
    if (!renderLevel.empty() && renderLevel.front() == '{') {
        // Convert renderLevel from "{5,4,3}" to "5_4_3"
        std::string convertedRenderLevel = renderLevel;
        convertedRenderLevel.erase(0, 1); // Remove the first character '{'
        convertedRenderLevel.erase(convertedRenderLevel.size() - 1); // Remove the last character '}'
        std::replace(convertedRenderLevel.begin(), convertedRenderLevel.end(), ',', '_');
        filename = baseDir + "/selective/levels_" + convertedRenderLevel + "_point_cloud.ply";
    } else {
        filename = baseDir + "/level/level_" + renderLevel + "_point_cloud.ply";
    }

	if (_sh_degree == 0)
	{
		count = loadPly<0>(filename.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
	}
	else if (_sh_degree == 1)
	{
		count = loadPly<1>(filename.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
	}
	else if (_sh_degree == 2)
	{
		count = loadPly<2>(filename.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
	}
	else if (_sh_degree == 3)
	{
		count = loadPly<3>(filename.c_str(), pos, shs, opacity, scale, rot, _scenemin, _scenemax);
	}
